...
export const projectApi = {
  ...
  // 获取项目日志
  getProjectLogs: async (id: string, lines: number = 50, logType: 'all' | 'key' | 'error' = 'all'): Promise<{logs: Array<{timestamp: string, module: string, level: string, message: string}>}> => {
    return api.get(`/projects/${id}/logs?lines=${lines}&log_type=${logType}`)
  },
  ...
  // 获取切片视频URL
  getClipVideoUrl: (projectId: string, clipId: string, title?: string): string => {
    // 直接使用clipId，让后端处理文件查找
    // title 参数暂未使用，可用于未来自定义文件名校验
...
// ========== Electron 配置适配层 ==========
// 在 Electron 环境下使用 IPC API，纯浏览器模式下使用后端 API
...
declare global {
  interface Window {
    electronAPI?: {
      ...
      updateFFmpegConfig: (config: any) => Promise<any>
      updateStorageConfig: (config: any) => Promise<any>
      updateApiConfig: (config: any) => Promise<any>
      updateProcessingConfig: (config: any) => Promise<any>
      detectFFmpeg: () => Promise<any>
      openConfigDir: () => Promise<any>
      openStorageDir: (dirType: string) => Promise<any>
...
// Electron 配置 API（仅在 Electron 环境下可用）
export const electronConfigApi = {
  // 获取完整配置
  getConfig: async () => {
    if (!isElectron()) {
  ...
  // 获取配置节
  getConfigSection: async (section: string) => {
    if (!isElectron()) {
      throw new Error('Not in Electron environment')
    }
    return await window.electronAPI!.getConfigSection(section)
  },

  // 更新 FFmpeg 配置
  updateFFmpegConfig: async (config: any) => {
    if (!isElectron()) {
      throw new Error('Not in Electron environment')
    }
    return await window.electronAPI!.updateFFmpegConfig(config)
  },
  ...
  updateStorageConfig: async (config: any) => {
    ...
    return await window.electronAPI!.updateStorageConfig(config)
  },

  // 更新 API 配置
  updateApiConfig: async (config: any) => {
    if (!isElectron()) {
      throw new Error('Not in Electron environment')
    }
  ...
  // 更新处理参数配置
  updateProcessingConfig: async (config: any) => {
    if (!isElectron()) {
      throw new Error('Not in Electron environment')
    }
    return await window.electronAPI!.updateProcessingConfig(config)
  },
  ...
  detectFFmpeg: async () => {
    ...
    return await window.electronAPI!.detectFFmpeg()
  },
  ...
  // 获取最近任务的日志列表
  getLogsList: async (limit: number = 20) => {
    return api.get('/logs', { params: { limit } })
  },

  // 获取指定项目的日志
  getProjectLogs: async (projectId: string, limit: number = 100) => {
    return api.get(`/logs/${projectId}`, { params: { limit } })
  },

  // 获取实时日志（轮询用）
  getRealtimeLogs: async (projectId?: string, lines: number = 100) => {
    return api.get('/logs/realtime/tail', { params: { project_id: projectId, lines } })
  },

  // 删除指定项目的日志
  deleteProjectLogs: async (projectId: string) => {
    return api.delete(`/logs/${projectId}`)
  },
  ...
  createLogStream: (projectId?: string): EventSource => {
    const baseURL = api.defaults.baseURL?.replace('http://localhost:8000', 'http://localhost:8000') || 'http://localhost:8000'
    const url = projectId
      ? `${baseURL}/logs/stream?project_id=${encodeURIComponent(projectId)}`
      : `${baseURL}/logs/stream`
