...
const BilibiliDownload: React.FC<BilibiliDownloadProps> = ({ onDownloadSuccess }) => {
  ...
  const parseVideoInfo = async () => {
    ...
    try {
      ...
      // 显示更详细的错误信息，而不是统一提示
      let errorMessage = '请输入正确的视频链接'
      if (error && typeof error === 'object' && 'response' in error) {
        const axiosError = error as { response?: { data?: { detail?: string } } }
        if (axiosError.response?.data?.detail) {
          // 从后端返回的详细错误信息
          const detail = axiosError.response.data.detail
          if (detail.includes('No video formats found') || detail.includes('cookies')) {
            errorMessage = '视频需要登录才能下载，请检查设置中的B站cookies配置'
          } else if (detail.includes('HTTP Error 404')) {
            errorMessage = '视频不存在或已被删除'
          } else if (detail.includes('解析视频信息失败')) {
            errorMessage = detail.replace('解析视频信息失败: ', '').split('\n')[0]
          } else {
            errorMessage = detail
          }
        }
        ...
        if (err.message?.includes('Network Error') || err.message?.includes('ECONNREFUSED')) {
          errorMessage = '无法连接到后端服务，请重启应用'
        } else {
          errorMessage = err.message || '请输入正确的视频链接'
        }
      }
