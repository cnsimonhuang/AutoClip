...
class ProjectManager:
    ...
    def validate_project_exists(self, project_id: str) -> bool:
        ...
        exists = project_base.exists()
        logger.info(f"[DEBUG] validate_project_exists: project_id={project_id}")
        logger.info(f"[DEBUG] uploads_dir={self.config.get_path_config().uploads_dir}")
        logger.info(f"[DEBUG] project_base={project_base}, exists={exists}")
        logger.info(f"[DEBUG] AUTO_CLIP_DATA_DIR={os.environ.get('AUTO_CLIP_DATA_DIR', 'NOT SET')}")
        logger.info(f"[DEBUG] AUTO_CLIP_UPLOADS_DIR={os.environ.get('AUTO_CLIP_UPLOADS_DIR', 'NOT SET')}")
        return exists
