...
# 项目根目录 - 支持 macOS SIP 限制，使用环境变量指定用户数据目录
import os

def _get_user_base_dir():
    """获取用户数据基准目录（不包含data子目录）"""
    # AUTO_CLIP_DATA_DIR 是 data 子目录，需要取其父目录
...
PROJECT_ROOT = _get_user_base_dir()
...
# Prompt文件路径 - prompt目录仍然使用 app bundle 内的路径
_APP_BUNDLE_ROOT = Path(__file__).parent.parent
PROMPT_DIR = _APP_BUNDLE_ROOT / "prompt"
PROMPT_FILES = {
...
class PathConfig:
    """路径配置"""
    project_root: Path = field(default_factory=lambda: _get_user_base_dir())
    data_dir: Path = field(default_factory=lambda: _get_user_base_dir() / "data")
    uploads_dir: Path = field(default_factory=lambda: _get_user_base_dir() / "uploads")
    output_dir: Path = field(default_factory=lambda: _get_user_base_dir() / "output")
    prompt_dir: Path = field(default_factory=lambda: _APP_BUNDLE_ROOT / "prompt")
    temp_dir: Path = field(default_factory=lambda: _get_user_base_dir() / "temp")
...
class ConfigManager:
    ...
    def get_project_paths(self, project_id: str) -> Dict[str, Path]:
        ...
        uploads_dir = path_config.uploads_dir
        project_base = uploads_dir / project_id
        
        # 调试日志
        import os
        logger.info(f"[DEBUG get_project_paths] project_id={project_id}")
        logger.info(f"[DEBUG get_project_paths] path_config.uploads_dir={uploads_dir}")
        logger.info(f"[DEBUG get_project_paths] project_base={project_base}")
        auto_clip_data_dir = os.environ.get('AUTO_CLIP_DATA_DIR', 'NOT SET')
        logger.info(f"[DEBUG get_project_paths] AUTO_CLIP_DATA_DIR={auto_clip_data_dir}")
        if auto_clip_data_dir != 'NOT SET':
            logger.info(f"[DEBUG get_project_paths] Path(AUTO_CLIP_DATA_DIR).parent={Path(auto_clip_data_dir).parent}")
